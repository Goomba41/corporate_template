<template>
    <div :class="progressClasses">
        <!-- 
          Слот для полной кастомизации отображения прогресс-бара.
          Предоставляет данные: progress (нормализованное значение), value (исходное), max.
          Если слот не переопределён — отображается дефолтная реализация ниже.
        -->
        <slot
            name="custom"
                ...
                >
                    ...
                    >
                        <!-- 
                          ...
                          Если слот передан — отображаем его содержимое.
                        -->
 ...
 * - Гибко настраивается через слоты и CSS-переменные
 ...
 * ```
 * 
 * @slot custom - Полная замена визуальной части. Получает: { progress: number, value: number | null, max: number }
...
watch(
    ...
    { immediate: true } // Запускается сразу при подключении компонента + следит за изменениями
)
